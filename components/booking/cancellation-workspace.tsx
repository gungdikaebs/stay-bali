...
export async function CancellationWorkspace() {
  ...
  return (
    <section className="mx-auto mt-8 max-w-[1500px] rounded-2xl border border-border bg-white p-5 shadow-sm sm:p-6">
        ...
        {requests.map((request) => (
          <article className="rounded-2xl border border-border p-5" key={request.id}>
            <div className="flex flex-wrap items-start justify-between gap-3">
              ...
              <span className="inline-flex items-center gap-1">{request.status === "REJECTED" ? <Ban className="size-3.5 text-destructive" aria-hidden="true" /> : <BadgeCheck className="size-3.5 text-primary" aria-hidden="true" />}<StatusBadge status={request.status} /></span>
            </div>
      ...
      </div> : <div className="mt-6 rounded-2xl bg-secondary/50"><EmptyState description="Traveler cancellation requests requiring review will appear here." icon={RotateCcw} title="No cancellation requests" /></div>}
    </section>
  );
}
